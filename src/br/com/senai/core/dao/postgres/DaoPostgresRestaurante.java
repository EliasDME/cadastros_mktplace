...
public class DaoPostgresRestaurante implements DaoRestaurante {
	...
	private final String INSERT = "INSERT INTO (nome, descricao, cidade, logradouro,"
	...
	private final String BUSCAR_BY_ID = "SELECT r.i id_restaurante, r.nome nome_restaurante,"
	...
	private final String BUSCAR_BY_NAME_CATEGORIA = "SELECT r.i id_restaurante, r.nome nome_restaurante,"
			+ " r.descricao, r.cidade r.logradouro, r.bairro, r.complemento,"
			+ " c.id id_categoria, c.nome nome_categoria"
			+ "	FROM restaurantes r, categorias c"
			+ " WHERE r.id_categoria = c.id";
	...
	public List<Restaurante> listarPor(String nome, Categoria categoria) {
		...
		try {
			StringBuilder consulta = new StringBuilder(BUSCAR_BY_NAME_CATEGORIA);
			...
			if(nome != null && !nome.isBlank()) {
				consulta.append(" AND Uppe(r.nome) LIKE Upper(?) ");
			}
			...
			while(rs.next()) {
				restaurantes.add(extrairDoResultSet(rs));
			}
			return restaurantes;
	...
	private Restaurante extrairDoResultSet(ResultSet rs) {
		try {
			...
			Endereco endereco = new Endereco(nomeCategoria, logradouro, bairro, complemento);
